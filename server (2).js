...
// ── /api/inventory ────────────────────────────────────────────────────────────
// Extract qty from a snapshot — handles multiple Unicommerce field names
function extractQty(snap) {
  return snap.inventory ?? snap.totalQty ?? snap.quantity ??
         snap.availableQty ?? snap.physicalQty ?? snap.openQty ?? 0;
}

// Try multiple Unicommerce API endpoints until one returns data
async function tryGetInventory(cfg, fac) {
  const endpoints = [
    { path: "/services/rest/v1/inventory/inventorySnapshot/get",
      body: { itemTypeSKUs: null, updatedSinceInMinutes: null, facilityCode: fac },
      pick: b => b.inventorySnapshots || b.inventorySnapshot || [] },
    { path: "/services/rest/v1/inventory/itemInventory/get",
      body: { facilityCode: fac, itemTypeSKUs: null },
      pick: b => b.itemInventories || b.inventories || b.items || [] },
    { path: "/services/rest/v1/inventory/shelf/get",
      body: { facilityCode: fac },
      pick: b => b.shelves || b.shelfInventory || b.inventories || [] },
  ];
  for (const ep of endpoints) {
    try {
      const r = await ucReq(cfg, "POST", ep.path, ep.body, fac);
      console.log(`[UC] ${ep.path} | ${fac} | HTTP ${r.status} | body[0-150]: ${JSON.stringify(r.body).slice(0,150)}`);
      if (r.ok && r.body?.successful) {
        const snaps = ep.pick(r.body);
        if (snaps.length > 0) return { snaps, source: ep.path.split("/").pop() };
      }
    } catch(e) { console.warn(`[UC] ${ep.path} failed:`, e.message); }
  }
  return null;
}
...
async function fetchInventory(cfg) {
  ...
  await Promise.all(Object.entries(cfg.facilities).map(async ([city, fac]) => {
    try {
      const data = await tryGetInventory(cfg, fac);

      if (!data) {
        const msg = `All inventory endpoints failed for "${fac}". Check facility code & credentials.`;
        warnings.push(`${city}: ${msg}`);
        syncLogRows.push({ syncType:"inventory", city, facility:fac, apiStatus:"all-failed",
          totalItems:0, matchedSkus:0, unmatchedSkus:0, unmatchedSample:[], mappedData: msg });
        return;
      }

      const { snaps, source } = data;
      const unmatched = [];

      for (const snap of snaps) {
        const skuVal  = snap.itemTypeSKU || snap.SKU || snap.sku || snap.itemSKU || snap.code || "";
        const nameVal = snap.itemTypeName || snap.name || snap.itemName || snap.description || "";
        const short   = matchFlavour(skuVal) || matchFlavour(nameVal);
        if (!short) {
          unmatched.push((skuVal || nameVal || "?").slice(0, 60));
          continue;
        }
        result[city][short] = (result[city][short] || 0) + extractQty(snap);
      }

      console.log(`[UC] Inventory ${city} via ${source}: ${snaps.length} items → ${Object.keys(result[city]).length} SKUs matched`);
      if (unmatched.length) console.log(`[UC] ${city} unmatched (first 10):`, unmatched.slice(0,10));

      syncLogRows.push({
        syncType: "inventory("+source+")", city, facility: fac,
        apiStatus: "200", totalItems: snaps.length,
      ...
      console.error(`[UC] Inventory ${city} error:`, e.message);
      syncLogRows.push({ syncType:"inventory", city, facility:fac, apiStatus:"error",
        totalItems:0, matchedSkus:0, unmatchedSkus:0, unmatchedSample:[], mappedData: e.message });
    }
  }));
...
// ══════════════════════════════════════════════════════════════════════════════
// DAILY AUTO-SYNC — runs at 09:00 IST (03:30 UTC) every day
// Uses node-cron. Install: npm install node-cron
// ══════════════════════════════════════════════════════════════════════════════

// In-memory cache for last auto-sync result
let autoSyncCache = {
  lastRun:    null,   // ISO timestamp
  lastStatus: null,   // "success" | "failed"
  lastError:  null,
  inventory:  { BLR:{}, KOL:{}, DEL:{} },
  demand:     { BLR:{}, KOL:{}, DEL:{} },
  dateRange:  null,
  warnings:   [],
};

// Sheet URL for auto-sync logging — set via env var or posted from app
let storedSheetUrl = process.env.SHEET_URL || "";

// Allow app to register sheet URL once (called from Settings save)
app.post("/api/register-sheet", (req, res) => {
  const { sheetUrl } = req.body || {};
  if (sheetUrl) { storedSheetUrl = sheetUrl; console.log("[Auto] Sheet URL registered"); }
  res.json({ success: true });
});

// Return the last cached auto-sync result
app.get("/api/last-sync", (req, res) => {
  res.json({ success: true, ...autoSyncCache });
});

// Manually trigger a sync (also called by the cron)
async function runAutoSync() {
  const cfg = getConfig({});            // uses env var defaults
  const runAt = new Date().toISOString();
  console.log(`[Auto] Starting sync at ${runAt} IST`);
  try {
    const [inv, ord] = await Promise.all([
      fetchInventory(cfg),
      fetchOrders(cfg, 1),              // today's sales
    ]);
    autoSyncCache = {
      lastRun:    runAt,
      lastStatus: "success",
    ...
    console.log(`[Auto] Sync OK — inventory cities: ${Object.keys(inv.inventory).join(",")}`);

    // Push summary to Google Sheet if URL is registered
    if (storedSheetUrl) {
      const rows = Object.entries(inv.inventory).map(([city, skuMap]) => ({
        syncType: "auto-9am", city, facility: cfg.facilities[city] || city,
        apiStatus: "200",
        totalItems: Object.values(skuMap).reduce((s,v)=>s+v,0),
        matchedSkus: Object.keys(skuMap).length,
        unmatchedSkus: 0, unmatchedSample: [],
        mappedData: Object.entries(skuMap).map(([k,v])=>k+":"+v).join(", "),
      }));
      postToSheet(storedSheetUrl, "logUCSync", { rows }).catch(()=>{});
    }
  } catch(e) {
    autoSyncCache.lastRun    = runAt;
    autoSyncCache.lastStatus = "failed";
    autoSyncCache.lastError  = e.message;
    console.error("[Auto] Sync FAILED:", e.message);
  }
}

// POST /api/trigger-sync — manually trigger from the app
app.post("/api/trigger-sync", async (req, res) => {
  const { sheetUrl } = req.body || {};
  if (sheetUrl) storedSheetUrl = sheetUrl;
  await runAutoSync();
  res.json({ success: autoSyncCache.lastStatus === "success", ...autoSyncCache });
});
...
try {
  ...
  cron.schedule("30 3 * * *", () => {
    console.log("[Auto] Cron fired — 09:00 IST daily sync");
    runAutoSync();
  }, { timezone: "UTC" });
  console.log("[Auto] Cron scheduled: 09:00 IST daily (03:30 UTC)");
} catch(e) {
  console.warn("[Auto] node-cron not installed — daily sync disabled. Run: npm install node-cron");
}
