...
async function getToken() {
  ...
  // Pass credentials both as query params AND headers — UC accepts either
  const encodedUser = encodeURIComponent(UC_USER);
  const encodedPass = encodeURIComponent(UC_PASS);
  const url = `${BASE_URL}/oauth/token?grant_type=password&client_id=my-trusted-client&username=${encodedUser}&password=${encodedPass}`;

  console.log("Fetching UC token for user:", UC_USER, "tenant:", TENANT);
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = https.request(options, (res) => {
      ...
      res.on("data", chunk => data += chunk);
      res.on("end", () => {
        console.log("UC auth response status:", res.statusCode);
        console.log("UC auth response body:", data.slice(0, 300));
        try {
          const json = JSON.parse(data);
          if (!json.access_token) {
            return reject(new Error("Auth failed (status " + res.statusCode + "): " + JSON.stringify(json)));
          }
          tokenCache.accessToken = json.access_token;
          tokenCache.expiresAt   = now + ((json.expires_in || 3600) - 300) * 1000;
          console.log("UC token obtained, expires in", json.expires_in, "sec");
          resolve(json.access_token);
        } catch(e) {
          reject(new Error("Auth parse error: " + e.message + " | raw: " + data.slice(0, 200)));
        }
      });
    });
    req.on("error", (e) => {
      console.error("UC auth network error:", e.message);
      reject(e);
    });
    req.end();
  });
...
// Render deploys to /opt/render/project/src — handle both layouts
const STATIC_DIR = (() => {
  const candidates = [
    __dirname,
  ...
  for (const d of candidates) {
    if (fs.existsSync(path.join(d, "index.html"))) {
      console.log("Found index.html in:", d);
      return d;
    }
  }
  console.warn("index.html not found in any candidate, using __dirname:", __dirname);
  return __dirname;
})();
...
app.get("*", (req, res) => {
  if (fs.existsSync(INDEX_FILE)) {
    ...
    res.status(500).send("index.html not found in " + STATIC_DIR);
  }
});

const PORT = process.env.PORT || 3000;
app.listen(PORT, () => {
  console.log(`KILRR proxy running on port ${PORT}`);
  console.log(`Tenant: ${TENANT}`);
  console.log(`UC user configured: ${UC_USER ? "yes" : "NO - check env vars"}`);
});
